...
namespace ShoppingCartService
{
    ...
    internal sealed class ShoppingCartService : StatefulService, IShoppingCartService
    {
        ...
        {
            var cart = await this.StateManager
                .GetOrAddAsync<IReliableDictionary<string, ShoppingCartItem>>("myCart");
            using (var tx = this.StateManager.CreateTransaction())
            {
        ...
        public async Task<List<ShoppingCartItem>> GetItems()
        {
            ...
            var retList = new List<ShoppingCartItem>();

            using (var tx = this.StateManager.CreateTransaction())
            {
                var enumAsync = await cart.CreateEnumerableAsync(tx);
                var list = await enumAsync.ToListAsync(tx);
